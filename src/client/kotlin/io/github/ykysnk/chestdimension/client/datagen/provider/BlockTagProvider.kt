...
class BlockTagProvider(output: FabricDataOutput, registries: CompletableFuture<HolderLookup.Provider>) :
    ...
    override fun addTags(provider: HolderLookup.Provider) {
        getOrCreateTagBuilder(MCBlockTags.WALLS).apply {
            add(Blocks.CHEST_PLATFORM_WALL)
        }
        getOrCreateTagBuilder(MCBlockTags.FENCES).apply {
            add(Blocks.CHEST_PLATFORM_FENCE)
        }
        getOrCreateTagBuilder(MCBlockTags.PRESSURE_PLATES).apply {
            add(Blocks.TELEPORT_PRESSURE_PLATE)
        }
        getOrCreateTagBuilder(MCBlockTags.WOODEN_PRESSURE_PLATES).apply {
            add(Blocks.TELEPORT_PRESSURE_PLATE)
        }
        getOrCreateTagBuilder(MCBlockTags.WALL_POST_OVERRIDE).apply {
            add(Blocks.TELEPORT_PRESSURE_PLATE)
            add(Blocks.CHEST_PLATFORM_ENTER_PLATE)
        }
        getOrCreateTagBuilder(MCBlockTags.MINEABLE_WITH_AXE).apply {
        ...
        getOrCreateTagBuilder(MCBlockTags.MINEABLE_WITH_PICKAXE).apply {
            add(Blocks.DEATH_BODY)
        }
        getOrCreateTagBuilder(BlockTags.GLASS_PANES).apply {
            add(Blocks.BLAST_RESISTANT_GLASS_PANE)
        }
    }
}